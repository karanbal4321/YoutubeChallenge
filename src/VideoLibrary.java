...
class VideoLibrary {
    ...
    private final ArrayList<Video> nonFlaggedVideos = new ArrayList<>();
    private Video videoCurrentlyPlaying = null;
    ...
    VideoLibrary() {
        ...
        try {
            ...
            while (scanner.hasNextLine()) {
                ...
                this.nonFlaggedVideos.add(new Video(title,id,tags));
            }
    ...
    Video getVideoCurrentlyPlaying() {
        return this.videoCurrentlyPlaying;
    }

    void setVideoCurrentlyPlaying(Video video) {
        this.videoCurrentlyPlaying = video;
    }

    ArrayList<Video> getNonFlaggedVideos() {
        return nonFlaggedVideos;
    }

    void removeFlaggedVideo(String videoId) {
        Video videoToRemove = null;
        for (Video vid: nonFlaggedVideos) {
            if (vid.getVideoId().equals(videoId)) {
                videoToRemove = vid;
            }
        }
        nonFlaggedVideos.remove(videoToRemove);
    }
